defmodule Celeste.MP3.ID3v2p4 do
  ...
  def decode_frames(binary, acc) do
    ...
    frame =
      case encoding do
        0 -> frame_remainder |> verify_length_or_unterminate!(data_length, <<0>>)
        1 -> frame_remainder |> verify_length_or_unterminate!(data_length, <<0, 0>>) |> decode_utf16()
        2 -> frame_remainder |> verify_length_or_unterminate!(data_length, <<0, 0>>) |> decode_utf16be()
        3 -> frame_remainder |> verify_length_or_unterminate!(data_length, <<0>>)
        _ -> raise("unknown frame encoding")
      end
  ...
  defp decode_frame_flags(binary) do
    <<0::size(1), tag_preservation::size(1), file_preservation::size(1), read_only::size(1),
      0::size(1), 0::size(1), 0::size(1), 0::size(1),
      0::size(1), grouping::size(1), 0::size(1), 0::size(1),
      compression::size(1), encryption::size(1), unsync::size(1), data_length_indicator::size(1)>> = binary
  ...
  defp undo_unsynchronization!(<<c>> <> binary, acc), do: undo_unsynchronization!(binary, acc <> <<c>>)
  ...
  defp unterminate!(<<>>, terminator, _), do: raise("terminator missing")
  defp unterminate!(terminator, terminator, acc), do: acc
  defp unterminate!(<<c>> <> binary, terminator, acc), do: unterminate!(binary, terminator, acc <> <<c>>)
